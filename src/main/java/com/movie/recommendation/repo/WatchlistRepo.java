...
public interface WatchlistRepo extends JpaRepository<WatchList,Long> {
    @Query(value = "select w.watchlist_id,w.movie_id_fk,w.user_id_fk from watchlist_table w inner join user_table u on w.user_id_fk=u.user_id inner join movie_table m on m.movie_id=w.movie_id_fk where w.user_id_fk=?1 and w.user_id_fk=?2",nativeQuery = true)
